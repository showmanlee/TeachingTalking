...
public class SentenceCanvas : MonoBehaviour {
	public Text message;
	...
	public void Typed() {
		...
		if (input.text == "" || input.text.Trim() == "")
			...
			CardCombine cc = new CardCombine(input.text, false, -1, true, input.text.Trim());
			GameManager.instance.cards[i - 1][j - 1].CombineAdd(cc, 3);
		}
		else
			// 선택한 카드가 여러 개일 경우
			GameManager.instance.MultiCardLearning(selectedCards, input.text, true);
		// 후처리
		foreach (var c in selectedCards)
			c.GetComponent<Card>().Unselect();
		selectedCards.Clear();
